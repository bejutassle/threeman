...
class PhpFilesLoader implements LoaderInterface
{
    ...
    public function loadMessages($force = false)
    {
        if (!isset($this->messages[$this->language]) || $force) {
            if ($this->isLanguageFileExists($this->language)) {
                $this->messages[$this->language] = require($this->path . '/' . $this->language . '/messages.php');
            } else {
    ...
    public function get($message)
    {
        ...
        if (empty($this->messages[$this->language]) || empty(array_get($this->messages[$this->language], $message))) {
            throw new NotFoundException();
        }
        return array_get($this->messages[$this->language], $message);
    }
    ...
    {
        return isset($this->messages[$this->language]) && !is_null(array_get($this->messages[$this->language], $message));
    }
}
