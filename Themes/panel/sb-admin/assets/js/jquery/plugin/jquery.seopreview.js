    ...
    "use strict";
     ...
     $.extend({ seoPreview: function (options) {
        ...
        var defaults = {
            ...
            metadata: {
                title: "Title",
                desc: "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugit incidunt, fugiat cumque enim illo repellendus tempora porro? Eius delectus sunt incidunt quos fugiat! Illum nostrum, perferendis, necessitatibus similique velit nobis.",
                _placement: false,
                _title: false,
                _separator: false,
                url: {
        ...
        function truncate(original_text, limit, sets, status) {
            if(!sets) sets = false;
            ...
            if (!sets['metadata']){
             ...
             var meta = sets['metadata'];
            }

            if(meta){
                var meta_placement = meta['_placement'];
                var meta_title = meta['_title'];
                var meta_separator = meta['_separator'];
            }
            ...
            if(status == true && meta_placement && meta_title && meta_separator){
                    if(meta_placement == '1'){
                        truncated_fix +=  meta_title+" "+meta_separator+" "+truncated_text;
                    }else{
                        truncated_fix +=  truncated_text+" "+meta_separator+" "+meta_title; 
                    }
            }else{
                        truncated_fix += truncated_text;
            }


            return truncated_fix;
        }
        ...
        $.each(settings.metadata, function (index, value) {
            if (value[0] && (value[0].type === "text" || value[0].type === "textarea")) {
                ...
                $(document).on("keyup", value, function () {
                    $(document).find("span#js-seo-preview__google-" + index).text(truncate(value.val(), character_limit, settings, type));
        ...
        google_box += "<span id='js-seo-preview__google-title'>" + truncate(metaTitle, 65, settings, true) + "</span>";
        if (settings.metadata.url.use_slug === true) {
